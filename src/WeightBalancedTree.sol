...
library WeightBalancedTree {
    ...
    struct Tree {
        ...
        uint256 root;
        mapping(uint256 => uint256) nodeMap;
    }

    /**
     * Returns the weight sum of the tree
     */
    function weightSum(Tree storage tree) public view returns (uint256) {
     ...
     */
    function insert(Tree storage tree, uint256 _id, uint256 _weight) internal {
     ...
     */
    function remove(Tree storage tree, uint256 _id) internal returns (bool) {
    ...
    function pullUp(Tree storage tree, uint256 probe) private {
        while (true) {
            if (tree.nodes[probe].leftChild == 0) {
    ...
    function update(Tree storage tree, uint256 _id, uint256 _weight) internal returns (bool) {
        ...
        uint256 oldWeight = tree.nodes[node].weight;
    ...
    function demote(Tree storage tree, uint256 probe) private {
        while (true) {
            if (tree.nodes[probe].leftChild != 0) {
                if (tree.nodes[probe].rightChild != 0) {
                    if (
